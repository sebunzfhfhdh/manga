...
router.get('/', async (req, res, next) => {
    ...
    try {
        const query = `SELECT mangas.* FROM mangas JOIN chapters ON mangas.id = chapters.manga_id GROUP BY mangas.id ORDER BY MAX(chapters.chapter_id) DESC;`;
        const result = await fetchManga(query);

        if (!result) {
            return res.status(404).json({ error: 'No latest manga found' });
        }
        ...
        console.error('Error fetching latest mangas:', err.message);
        next(err);
    }
});
